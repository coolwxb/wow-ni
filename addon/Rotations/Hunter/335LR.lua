local build = select(4, GetBuildInfo());  --获取版本信息
local wotlk = build == 30300 or false;  --判断是否wlk335
local filename = "335LR"  --文件名称
if wotlk then
local queue = {
	"FeedPet",
	"Pause",
...
local abilities = {
	...
	ni.bootstrap.profile(filename, queue, abilities, OnLoad, OnUnload);
else
    local queue = {
        "Error",
    };
    local abilities = {
        ["Error"] = function()
            ni.vars.profiles.enabled = false; --停止脚本
			if not wotlk then  --判断是否wlk335,不是则停止
				ni.frames.floatingtext:message("这个脚本配置只适用于335a wlk!")
            end
        end,
    };
    ni.bootstrap.profile(filename, queue, abilities);   --这里""里面要改为文件名称
end;